...
def check_filename(fname, args_list):
    ...
    for arg_ind in np.arange(0, len(args_list), 2):
        arg = str(args_list[arg_ind])
        arg_value = str(args_list[arg_ind + 1])
...
def order_examples(data_idx,input_dir,output_dir,output_name,input_fname_args):

    class set_args():
        def __init__(self,input_dir,output_dir,output_name,input_fname_args):
            ''' input_fname_args : list:[key1,val1,key2,val2] '''
            ...
            self.input_fname_args = input_fname_args
            self.output_dir = output_dir


    args = set_args(input_dir,output_dir,output_name,input_fname_args)
    print(args)
    ...
    example_list = zip(ordered_examples,ordered_values)
    example_list = sorted(example_list, key=(lambda x: x[0]))
    example_idx,forget_values = list(zip(*example_list))  # label

    return data_idx,forget_values
